...
req(1:25,:) = req([21,22,23,24,25,1,2,3,4,6,7,5,8,9,11,10,...
    14,20,15,13,12,18,16,19,17],:);
...
% the rowindex for each food in livsmedel.xlsl
% and the amount. Write to file Result.txt
Food = string(food_names(find(x)));
FoodIndex = find(x)+1;
Amount = x(find(x)) .* 100;
fileID = fopen('Result.txt','w');

%Loop to write all data to result.txt
for i = 1:length(Amount)    
    fprintf(fileID,'(%d) %s %s\t %0.4f\n',FoodIndex(i),...
        Food(i),':', Amount(i));
end
fprintf(fileID,'\n%s %12.4f','Totalt intag:',fval .* 100);
fclose(fileID);
