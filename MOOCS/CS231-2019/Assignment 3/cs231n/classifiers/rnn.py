...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # First we need to take input from CNN and send as initial hidden state for RNN
        # Transform image pixels to to initial hidden state, (1)
        h0, _ = affine_forward(features, W_proj, b_proj) #cache not necessary, features is our input
        ...
        if self.cell_type == 'rnn':
            ...
            lstm_outputs, cache_lstm = lstm_forward(embedded_captions, h0, Wx, Wh, b)
        ...
        dh, dW_vocab, db_vocab = temporal_affine_backward(dsoftmax, cache_scores)
    ...
    def sample(self, features, max_length=30):
        ...
        for step in range(max_length):
            #(2)
            if self.cell_type == 'rnn':
               next_h, _ = rnn_step_forward(word_embed, h0, Wx, Wh, b)
            else:
                next_h, next_c, _ = lstm_step_forward(word_embed, h0, next_c, Wx, Wh, b)
