...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
        ...
        elif transition == 'LA': # (head, dependent)
            ...
            raise Exception("The transition was neither S, LA or RA.")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        predicted_transitions = model.predict(mini_batch)

        for idx, predicted_transition in enumerate(predicted_transitions):
            mini_batch[idx].parse_step(predicted_transition)

            if len(mini_batch[idx].buffer) == 0 and len(mini_batch[idx].stack) == 1:
                unfinished_parses.remove(mini_batch[idx])
